...
import { ChevronRight } from '@mui/icons-material'; // Import ChevronRight icon from Material-UI
...
const MUITABLE2 = ({ selectedTable, selectedSchema, onRetrivedRowData, tableCrud, isEdit: IsEdit }) => {
  ...
  const handleSort = (column) => {
    const isAsc = !column.isSortedDesc;
  ...
  const columns = useMemo(() => {
    ...
    const columnEdit = {
      ...
      cell: ({ row }) => (
        <div className='flex items-center'>
          <IconButton onClick={() => {
            debugger;
            setretrivedRowData(row.original);
            onRetrivedRowData(row.original, true);
          }}>
    ...
    dynamicColumns.push(columnEdit);
    ...
    fields.forEach(column => {
      dynamicColumns.push({
        ...
        cell: ({ row }) => {
          const cellData = row.original[column.column_name];
  ...
  const downloadExcel = async () => {
    debugger;
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_REACT_APP_API_URL}api/getExport/${selectedSchema}/${selectedTable}`);
      ...
      // Add column titles to the first row with corresponding cell coordinates
      columns1.forEach((column, index) => {
        ws.getCell(1, index + 1).value = column.title;
      });
      // Extract data values for each row
      response.data.forEach((rowData, rowIndex) => {
        columns1.forEach((column, columnIndex) => {
          const cellValue = rowData[column.field];
          ws.getCell(rowIndex + 2, columnIndex + 1).value = cellValue !== null ? cellValue.toString() : '';
        });
      });
      ...
      wb.xlsx.writeBuffer().then(buffer => {
        ...
        a.href = url;
        a.download = `${selectedSchema}_${selectedTable}.xlsx`;
  ...
  const table = useReactTable({
    ...
    initialState: {
      ...
      sortBy: [], // Initialize with an empty array for sorting
    },
  ...
  const loadFields = async (tableName) => {
    debugger;
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_REACT_APP_API_URL}api/getFields/${selectedSchema}/${selectedTable}`);
  ...
  const formatDates = async () => {
    ...
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_REACT_APP_API_URL}api/getForTables3/${selectedSchema}/${selectedTable}`);
    ...
    <>
      ...
      <Card>
        ...
        <div className="flex justify-between p-5 gap-4 flex-col items-start sm:flex-row sm:items-center">
          <Button color="secondary" variant="outlined" startIcon={<i className="ri-upload-2-line text-xl" />} onClick={() => downloadExcel()}>
            Export
          </Button>
        ...
        <TablePagination
          ...
          component="div"
          className="border-bs"
          count={totalRecords} // Update with the total count of rows from your backend
