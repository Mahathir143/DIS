...
const MUITable = () => {
  ...
  const [permissionTables, setPermissionTables] = useState([]);
  ...
  const validateTableName = (tableName) => {
    ...
    // For example, you can check against a list of valid table names
    // or against a database query
    // For now, let's assume all table names are valid
    return true;
  };
  ...
  const loadPermissionTable = async (schemaName, id) => {
    ...
    try {
      ...
      setCrudPermissions(response.data);

      console.log("Permission table data", response.data);
      const permissionTables = response.data.map(table => table.table);
      setPermissionTables(permissionTables);
  ...
  const loadRoleTable = async (schemaName, tableName) => {
    try {
      ...
      console.log('permissions data', crudPermissions);
  ...
  const handleRetrievedEdit = (isEdit) => {
    setOpen(isEdit);
  }
  ...
  return (
    <Grid container spacing={6}>
      ...
      <Grid item xs={12}>
        <Card>
          {permissionTables.includes(selectedTable) ? (
            ...
            <Typography variant='h5'>
              {selectedTable.replace(/_/g, ' ').replace(/\b\w/g, firstLetter => firstLetter.toUpperCase())} is not found!
            </Typography>
          )}
      ...
      >
        <Fade in={open}>
          <Box sx={style}>
            ...
            <FormLayoutsBasic FormFields={fields} selectedTable={selectedTable} selectedSchema={selectedSchema} editData={retrievedEditData} isEdit={IsEdit} onRetrivedEdit={handleRetrievedEdit} />
          </Box>
        </Fade>
      </Modal>
    </Grid>
